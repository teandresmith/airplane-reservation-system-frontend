...
const Return = () => {
  ...
  const selectedFlights = useAppSelector(
    (state) => state.booking.selectedFlights
  )
  ...
  const handleConfirmation = () => {
    dispatch(
      setSelectedFlights({ selectedFlights: [...selectedFlights, ticket] })
    )
  ...
  return (
    <Box component='div'>
      <Stack direction='row' justifyContent={'space-between'}>
        <Typography variant='h4'>{`Return ${booking?.to?.acronym}・${booking?.from?.acronym}`}</Typography>
        <Select
          color='secondary'
          defaultValue={'Price'}
          sx={{
      ...
      <Stack direction='row' spacing={1} sx={{ pb: 3 }}>
        <Box component='div'>
          <Typography variant='h5'>
            {booking?.roundTripReturnDate?.toString().slice(0, 15)}
          </Typography>
      ...
      <Stack direction='column' spacing={2}>
        {flightInfo.map((item, key) => (
          ...
          >
            ...
            {item.seatTypes.map((seat: any, key) => (
